...
(function () {
  ...
  function setSectionState(section, isOpen) {
    ...
    const iconWrap = headBtn.querySelector('.lab-section-head__icon');
    section.setAttribute('data-collapsed', isOpen ? 'false' : 'true');
    section.classList.toggle('is-tree-expanded', isOpen);
    ...
    content.setAttribute('data-expanded', isOpen ? 'true' : 'false');
    if (iconWrap) iconWrap.classList.toggle('is-expanded', isOpen);
  }
  ...
  function setupCollapsibleSections() {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      clip.className = 'lab-rd-tree-expand__clip';
      const treeBranch = document.createElement('div');
      treeBranch.className = 'lab-rd-tree-branch';
      treeBranch.appendChild(inner);
      clip.appendChild(treeBranch);
      content.classList.add('lab-rd-tree-expand');
  ...
  function renderSectionFlows() {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      triggerIcon.setAttribute('data-tree-chevron', '');
      ...
      branchHost.className = 'lab-rd-tree-expand lab-section-flow__expand';
      branchHost.setAttribute('data-tree-branch-host', '');
      branchHost.setAttribute('data-expanded', 'false');
      branchHost.setAttribute('aria-hidden', 'true');
      const clip = document.createElement('div');
      clip.className = 'lab-rd-tree-expand__clip';
      const treeBranch = document.createElement('div');
      treeBranch.className = 'lab-rd-tree-branch';
      ...
      treeBranch.appendChild(panel);
      clip.appendChild(treeBranch);
      branchHost.appendChild(clip);

      if (window.DisruptaLabRdExpand && typeof window.DisruptaLabRdExpand.setupTreePanel === 'function') {
        window.DisruptaLabRdExpand.setupTreePanel(trigger, branchHost, false);
      } else {
        trigger.addEventListener('click', (ev) => {
          const btn = ev.currentTarget;
          const expanded = btn.getAttribute('aria-expanded') === 'true';
          const host = btn.parentNode && btn.parentNode.querySelector('[data-tree-branch-host]');
          const icon = btn.querySelector('[data-tree-chevron]');
          const open = !expanded;
          btn.setAttribute('aria-expanded', open ? 'true' : 'false');
          btn.setAttribute(
            'aria-label',
            open ? 'Colapsar explicação do papel desta secção' : 'Expandir explicação do papel desta secção',
          );
          if (icon) icon.textContent = open ? '▾' : '▸';
          if (host) {
            host.setAttribute('data-expanded', open ? 'true' : 'false');
  ...
  function executeGeneration() {
    ...
    runProcessingSequence(() => {
      ...
      const highlightEl = document.getElementById('lab-rd-prompt-highlight');
      ...
      if (highlightEl) highlightEl.textContent = t;
      ...
      if (resultPanel || dossier) {
        ...
        const scrollTarget = resultPanel || dossier;
...
/**
 * Estado visual do comando central (hero) — foco / conteúdo.
 */
(function initLabCenterChatContext() {
  ...
  function updateCommandState() {
    const engaged = commandEl.contains(document.activeElement) || hasValue();
    commandEl.classList.toggle('lab-center-preview__command--engaged', engaged);
  }

  commandEl.addEventListener('focusin', updateCommandState);
  commandEl.addEventListener('focusout', () => {
    window.requestAnimationFrame(updateCommandState);
  });
  ta.addEventListener('input', updateCommandState);
  updateCommandState();
})();
