...
const ScannerCardStream = ({
  ...
  useEffect(() => {
    ...
    const updateCardEffects = () => {
      ...
      const cardTotalWidth = 400 + config.cardGap;
      ...
      cardLine.querySelectorAll<HTMLElement>(".card-wrapper").forEach((wrapper, index) => {
        ...
        if (cardX < scannerRight && (cardX + 400) > scannerLeft) {
          ...
          if (config.scanEffect === 'scramble' && wrapper.dataset.scanned !== 'true') {
    ...
    const animate = (currentTime: number) => {
      ...
      if (!isPaused && !cardStreamState.current.isDragging) {
        if (cardStreamState.current.velocity > cardStreamState.current.minVelocity) {
            cardStreamState.current.velocity *= config.friction;
        }
  ...
  }, [isPaused, cards, config]);

  return (
    <>
      <div 
        className="relative w-full h-[400px] flex items-center justify-center overflow-hidden bg-ds-bg-1 border-y border-white/5 transition-all duration-300 mx-auto"
        style={{ 
          maxWidth: `${config.viewportWidth}px`,
          paddingLeft: `${config.lateralPadding}px`, 
          paddingRight: `${config.lateralPadding}px`,
          transform: `translate(${config.offsetX}px, ${config.offsetY}px) scale(${config.scale}) rotate(${config.rotation}deg)`
        }}
      >
        <style jsx global>{`
          ...
          <div ref={cardLineRef} className="flex items-center whitespace-nowrap select-none will-change-transform" style={{ gap: `${config.cardGap}px` }}>
            {cards.map(card => (
              <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
                <div className="card-normal card absolute top-0 left-0 w-full h-full rounded-sm overflow-hidden bg-transparent shadow-[0_15px_40px_rgba(0,0,0,0.4)] z-[2] [clip-path:inset(0_0_0_var(--clip-right,0%))]">
                  <img src={card.image} alt="Card" className="w-full h-full object-cover rounded-sm transition-all duration-300 ease-in-out brightness-110 contrast-110 shadow-[inset_0_0_20px_rgba(0,0,0,0.1)]" />
                </div>
                <div className="card-ascii card absolute top-0 left-0 w-full h-full rounded-sm overflow-hidden bg-transparent z-[1] [clip-path:inset(0_calc(100%-var(--clip-left,0%))_0_0)]">
      ...
      <div className="pointer-events-auto">
        {config.showConsole && (
          <div className="fixed bottom-10 left-10 z-[9999] w-80 max-h-[80vh] overflow-y-auto p-6 bg-black/80 backdrop-blur-3xl border border-white/20 rounded-lg shadow-2xl select-none custom-scrollbar">
            ...
            <div className="space-y-8">
                  ...
                  <div className="flex gap-2">
                    {[1, -1].map(dir => (
                      <button 
                        ...
                        className={`px-3 py-1 rounded-sm font-mono text-[10px] uppercase border ${config.direction === dir ? 'bg-[#29BDF2] text-black border-[#29BDF2]' : 'border-white/20 text-white/40'}`}
                      >
              ...
              <div className="pt-4 border-t border-white/5 space-y-4">
                ...
                />
                <ScannerSlider 
                  label="Axial Rotation" value={config.rotation} min={-360} max={360} step={1}
                  ...
                  <div className="flex gap-2">
                    {['clip', 'scramble'].map(mode => (
                      <button 
                        ...
                        className={`px-3 py-1 rounded-sm font-mono text-[10px] uppercase border ${config.scanEffect === mode ? 'bg-white text-black border-white' : 'border-white/20 text-white/40'}`}
                      >
                        {mode}
                      </button>
                    ))}
        ...
        {!config.showConsole && (
          <button 
            onClick={() => setConfig(c => ({...c, showConsole: true}))}
            className="fixed bottom-10 left-10 z-[9999] px-4 py-2 bg-black/60 backdrop-blur-xl border border-white/10 rounded-sm font-mono text-[10px] tracking-widest text-white/60 hover:text-white shadow-xl"
          >
            OPEN OPTICAL_ENGINE
          </button>
        )}
...
const ScannerSlider = ({ label, value, min, max, step, onChange }: any) => (
  <div className="space-y-2">
    ...
    <input 
      ...
      onChange={(e) => onChange(parseFloat(e.target.value))}
      className="w-full h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-[#29BDF2]"
    />
  </div>
);
